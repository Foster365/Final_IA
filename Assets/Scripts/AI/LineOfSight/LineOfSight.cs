...
public class LineOfSight
{
    ...
    public Transform GetLineOfSight()
    {
        ...
        Target = null;
        Collider[] overlapSphere = Physics.OverlapSphere(visionPoint.position, viewDistance, sightTarget, QueryTriggerInteraction.Ignore);
    ...
    private void OnDrawGizmos()
    {
        ...
        Vector3 rightLimit = Quaternion.AngleAxis(viewCone, Vector3.up) * Vector3.forward;
        Gizmos.DrawLine(visionPoint.position, visionPoint.position + (rightLimit * viewDistance));

        Vector3 leftLimit = Quaternion.AngleAxis(-viewCone, Vector3.up) * Vector3.forward;
